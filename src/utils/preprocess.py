...
class CropAndExtract():
    def __init__(self, sadtalker_path, device):
        try:
            ...
            self.device = device
            print("CropAndExtract INIT OK")
        except Exception as e:
            print("Error occured", e)

    def generate(self, input_path, save_dir, crop_or_resize='crop', source_image_flag=False, pic_size=256):

        print(f"input_path is {input_path}, save_dir {save_dir} ")
        ...
        print(f"pic_name is {pic_name}, landmarks_path {landmarks_path} coeff_path {coeff_path} png_path {png_path} ")

        #load input
        if not os.path.isfile(input_path):
            raise ValueError('input_path must be a valid path to video/image file')
        elif input_path.split('.')[-1] in ['jpg', 'png', 'jpeg']:
            print("Loading PNG, JPG or JPEG")    
            ...
            try:
                ...
                fps = 25
                print("Successfully Loaded full_frames")    
            except Exception as e:
                print("Error while loading frames", e)
        else:
            print("Loading Video")            
            # loader for videos
        ...
        print ("landmarks_path is ", landmarks_path)
