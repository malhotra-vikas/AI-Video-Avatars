...
def init_path(cachePath, checkpoint_dir, config_dir, size=512, old_version=False, preprocess='crop'):
...
#    elif len(glob.glob(os.path.join(checkpoint_dir, '*.safetensors'))):
        ...
        sadtalker_paths = {
            "checkpoint":os.path.join(cachePath, checkpoint_dir, 'SadTalker_V0.0.2_'+str(size)+'.safetensors'),
            }
        use_safetensor = True
    else:
    ...
    if 'full' in preprocess:
        sadtalker_paths['mappingnet_checkpoint'] = os.path.join(cachePath, checkpoint_dir, 'mapping_00109-model.pth.tar')
        sadtalker_paths['facerender_yaml'] = os.path.join(config_dir, 'facerender_still.yaml')
    else:
        sadtalker_paths['mappingnet_checkpoint'] = os.path.join(cachePath, checkpoint_dir, 'mapping_00229-model.pth.tar')
